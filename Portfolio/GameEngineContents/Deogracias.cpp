...
void Deogracias::Start()
{
    ...
    LineUI_ = GetLevel()->CreateActor<LineUI>();
    LineUI_->CreateLine("그대는 성하를 고통으로부터 해방시켰다. 그리고 교황 성하는 지금 꿈의 저편으로 건너가,\n"
                           "원초의 행진을 함께하고 있다.");
    LineUI_->AddSound("DLG_0120_0.wav");
    LineUI_->CreateLine("비통한 기적의 요람, 최후의 성유물, 축복에 싸여 뒤틀리면서 조용히 한탄하고 슬퍼하는\n"
                           "아버지의 이마에서 새어나오는 피와 황금을 다음 성배.");
    LineUI_->AddSound("DLG_0120_1.wav");
    LineUI_->CreateLine(
        "첫 성유물은 성하가 기적의 고통의 입맞춤을 받은 뒤집힌 옥좌의 옆, \n이 잿더미 산의 꼭대기에 있다.");
    LineUI_->AddSound("DLG_0120_2.wav");
    LineUI_->CreateLine("그리고 지금, 기적과의 마지막 교분이 그대를 기다리고 있다.");
    LineUI_->AddSound("DLG_0120_3.wav");
    LineUI_->CreateLine("기적이 그대의 죄 많은 심장에 얼마나 스며들었는가는, 그대만이 아는 것이다.");
    LineUI_->AddSound("DLG_0120_4.wav");
    LineUI_->CreateLine("첫 번째 기적과의 교분이 그대를 기다리고 있다.");
    LineUI_->AddSound("DLG_0122_0.wav");
    LineUI_->SetFontColor(float4{0.63f, 0.6f, 0.55f});
    LineUI_->SetSpeed(10.f);
    LineUI_->Off();

    LineUI_->SetMassageEndEvent(6, [&]() { Penitent::GetMainPlayer()->SetIsFreezeEnd(true); });
}
