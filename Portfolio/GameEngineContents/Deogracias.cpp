...
void Deogracias::Start()
{
    ...
    MessageUI_->CreateLine("그대는 성하를 고통으로부터 해방시켰다. 그리고 교황 성하는 지금 꿈의 저편으로 건너가,\n"
                           "원초의 행진을 함께하고 있다.");
    MessageUI_->AddSound("DLG_0120_0.wav");
    MessageUI_->CreateLine("비통한 기적의 요람, 최후의 성유물, 축복에 싸여 뒤틀리면서 조용히 한탄하고 슬퍼하는\n"
    ...
    MessageUI_->CreateLine("그리고 지금, 기적과의 마지막 교분이 그대를 기다리고 있다.");
    MessageUI_->AddSound("DLG_0120_3.wav");
    MessageUI_->CreateLine("기적이 그대의 죄 많은 심장에 얼마나 스며들었는가는, 그대만이 아는 것이다.");
    MessageUI_->AddSound("DLG_0120_4.wav");
    MessageUI_->CreateLine("첫 번째 기적과의 교분이 그대를 기다리고 있다.");
