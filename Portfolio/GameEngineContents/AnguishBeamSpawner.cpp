...
{ 
	...
	Beam->GetTransform().SetWorldPosition(float4{1000, -800, static_cast<int>(ACTORORDER::BossMonsterEffect)});
...
void AnguishBeamSpawner::CreateTwice() 
{
	{
        AnguishBeam* Beam = GetLevel()->CreateActor<AnguishBeam>();
        Beam->GetTransform().SetWorldPosition(float4{500, -800, static_cast<int>(ACTORORDER::BossMonsterEffect)});
        Beam->MetaRenderer_->ChangeMetaAnimation("threeAnguishBigBeam");
	}

	{
        AnguishBeam* Beam = GetLevel()->CreateActor<AnguishBeam>();
        Beam->GetTransform().SetWorldPosition(float4{1500, -800, static_cast<int>(ACTORORDER::BossMonsterEffect)});
        Beam->MetaRenderer_->ChangeMetaAnimation("threeAnguishBigBeam");
	}
