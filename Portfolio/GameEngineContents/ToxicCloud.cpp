...
void ToxicCloud::Start()
{
    ...
    Renderer_->CreateFrameAnimationCutTexture("pope_toxicOrb_shoot", {"pope_toxicOrb.png", 8, 24, 0.07f, true});
    ...
    ExplsionV2_->AnimationBindEnd("pope_toxicCloudv2", [&](const FrameAnimation_DESC& _Info) { ExplsionV2_->Off(); });
    ExplsionV2_->GetTransform().SetWorldScale({150.f, 150.f, 1.f});
...
void ToxicCloud::ShootStart(const StateInfo& _Info) { Renderer_->ChangeFrameAnimation("pope_toxicOrb_create"); }
